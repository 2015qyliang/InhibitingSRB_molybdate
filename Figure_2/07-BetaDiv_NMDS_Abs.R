...
otuSampleDF = t(read.table('02-OTUtable.txt', header = T,sep = '\t',row.names = 1))
...
newdf = data.frame(data.scores, dclass = dclassDF$Group, 
                   TaxLevel = rep('OTU', nrow(data.scores)))

write.table(newdf, '08-NMDS_OTU.txt', append = F, quote = F,
...
write.table(data.frame(gpPairs, adonis.F, adonis.p, anosim.r, anosim.p, mrpp.delta, mrpp.p), 
            '10-CompositionTest_OTU.txt', append = F, quote = F, sep = '\t', 
