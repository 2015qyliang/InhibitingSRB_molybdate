...
randomDF = read.table('04-simuRandomDelt.txt', header = T, sep = '\t', stringsAsFactors = F)
...
ggsave('11-SimulationRandom.pdf', p, units = 'in', width = 3, height = 2.5)
...
write.table(data.frame(ComparePairs = cp, statisticD, Pvalue), '08-KStestRandom.txt', 
...
write.table(data.frame(ComparePairs = cp, statisticD, Pvalue), '08-WMtestRandom.txt', 
            append = F, quote = F, sep = '\t', row.names = F, col.names = T)
